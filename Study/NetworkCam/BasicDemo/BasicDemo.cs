...
namespace BasicDemo
{
    public partial class Form1 : Form
    {
        ...
        // ch:用于从驱动获取图像的缓存 | en:Buffer for getting image from driver | kor:드라이버에서 이미지를 가져오기 위한 버퍼
        ...
        // ch:显示错误信息 | en:Show error message | kor:오류 메시지 표시
        ...
        /************************************************************************
         *  @fn     IsColorData()
         *  @brief  判断是否是彩色数据 | 색상 데이터인지 확인
         *  @param  enGvspPixelType         [IN]           像素格式 | 픽셀 형식
         *  @return 成功，返回0；错误，返回-1 | @return 성공, 반환 0; 오류, 반환 -1
         ************************************************************************/
        ...
        {
            // ch:创建设备列表 | en:Create Device List | kor:장치 목록 생성
            System.GC.Collect();
            ...
            // ch:在窗体列表中显示设备名 | en:Display device name in the form list | kor:양식 목록에 장치 이름 표시
            ...
            // ch:选择第一项 | en:Select the first item | kor:첫 번째 항목 선택
        ...
        private void bnOpen_Click(object sender, EventArgs e)
        {
            ...
            // ch:获取选择的设备信息 | en:Get selected device information | kor:선택한 장치 정보 가져오기
            ...
            // kor:감지 네트워크 최적 패키지 크기(GigE 카메라에서만 작동)
            if (device.nTLayerType == MyCamera.MV_GIGE_DEVICE)
            {
            ...
            // ch:设置采集连续模式 | en:Set Continues Aquisition Mode | kor:계속 수집 모드 설정
            m_MyCamera.MV_CC_SetEnumValue_NET("AcquisitionMode", (uint)MyCamera.MV_CAM_ACQUISITION_MODE.MV_ACQ_MODE_CONTINUOUS);
            m_MyCamera.MV_CC_SetEnumValue_NET("TriggerMode", (uint)MyCamera.MV_CAM_TRIGGER_MODE.MV_TRIGGER_MODE_OFF);

            bnGetParam_Click(null, null);// ch:获取参数 | en:Get parameters | kor:매개변수 가져오기

            // ch:控件操作 | en:Control operation | kor:조작 제어
            SetCtrlWhenOpen();
        }
        ...
        {
            // ch:取流标志位清零 | en:Reset flow flag bit | kor:흐름 플래그 비트 재설정
            if (m_bGrabbing == true)
            {
                m_bGrabbing = false;
        ...
        {
            // ch:打开触发模式 | en:Open Trigger Mode | kor:트리거 모드 열기
            if (bnTriggerMode.Checked)
            {
                m_MyCamera.MV_CC_SetEnumValue_NET("TriggerMode", (uint)MyCamera.MV_CAM_TRIGGER_MODE.MV_TRIGGER_MODE_ON);

                // ch:触发源选择:0 - Line0; | en:Trigger source select:0 - Line0; | kor:트리거 소스 선택:0 - Line0;
                //           1 - Line1;
                //           2 - Line2;
                //           3 - Line3;
                //           4 - Counter;
        ...
        {
            // ch:标志位置位true | en:Set position bit true | kor:위치 비트를 참으로 설정
            m_bGrabbing = true;

            m_hReceiveThread = new Thread(ReceiveThreadProcess);
            m_hReceiveThread.Start();

            m_stFrameInfo.nFrameLen = 0;//取流之前先清除帧长度 | kor:스트림을 가져오기 전에 프레임 길이를 지웁니다.
            m_stFrameInfo.enPixelType = MyCamera.MvGvspPixelType.PixelType_Gvsp_Undefined;
            // ch:开始采集 | en:Start Grabbing | kor:잡기 시작
            ...
            // ch:控件操作 | en:Control Operation | kor:제어 작동
        ...
        private void cbSoftTrigger_CheckedChanged(object sender, EventArgs e)
        {
            if (cbSoftTrigger.Checked)
            {
                // ch:触发源设为软触发 | en:Set trigger source as Software | kor:트리거 소스를 소프트웨어로 설정
                m_MyCamera.MV_CC_SetEnumValue_NET("TriggerSource", (uint)MyCamera.MV_CAM_TRIGGER_SOURCE.MV_TRIGGER_SOURCE_SOFTWARE);
                if (m_bGrabbing)
                {
                    bnTriggerExec.Enabled = true;
        ...
        {
            // ch:触发命令 | en:Trigger command | kor:트리거 명령
        ...
        {
            // ch:标志位设为false | en:Set flag bit false | kor:플래그 비트를 거짓으로 설정
            m_bGrabbing = false;
            m_hReceiveThread.Join();

            // ch:停止采集 | en:Stop Grabbing | 그랩 중지
        ...
        // ch:去除自定义的像素格式 | en:Remove custom pixel formats | kor:사용자 정의 픽셀 형식 제거
